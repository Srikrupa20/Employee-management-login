...
CORS(app, resources={r"/auth/*": {
    "origins": ["http://localhost:5173", "https://employee-management-login.vercel.app","https://employeemanagement.entrepreneurshipnetwork.net"],
...
Leave Page
"""
@app.route('/auth/get_leave', methods=['GET'])
def get_leaves():
    leaves = db.leaves.find()
    result = []
    for leave in leaves:
        leave_data = {
            '_id': str(leave['_id']),  # Convert ObjectId to string
            'name': leave['name'],
            'employeeId': leave['employeeId'],
            'reason': leave['reason'],
            'numberOfDays': leave['numberOfDays'],
            'fromDate': leave['fromDate'].strftime('%Y-%m-%d'),
            'toDate': leave['toDate'].strftime('%Y-%m-%d')
        }
